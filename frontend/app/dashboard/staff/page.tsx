...
import { AddFacilityModal } from '@/components/ui/AddFacilityModal';
...
export default function StaffDashboard() {
  ...
  const handleMaintenanceClick = (facility: any) => {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Management portal for {user?.full_name}. Manage slots and maintenance schedules.
        </p>
      </div>
                ...
                {facilities.map((facility: any) => (
                  <div key={facility.id} className="border rounded-xl p-4 bg-card hover:shadow-md transition-shadow">
                    ...
                    <div className="mt-4 flex gap-2">
                        ...
                        <button 
                          onClick={() => handleMaintenanceClick(facility)}
                          className="flex-1 py-1.5 text-xs border rounded-md hover:bg-muted transition-colors flex items-center justify-center gap-1"
                        >
                          <HiCog /> Maintenance
                        </button>
                    </div>
